    ...
    public class Operator
    {
        ...
        public void Mine(object skp)
        {
            while (IsMining)
            {
                ...
                if (b != null)
                    Log.NewLine($"Solved block {b.Index} with nonce {b.Nonce} ({b.Hash.Substring(0, 10)}) in {(int)Date.Now().Subtract(started).TotalMinutes} mins! Target diff was: {Blockchain.GetInaccurateDifficulty()}.");
